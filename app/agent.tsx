...
export default function Agent() {
  ...
  async function runAction(action: any) {
    try {
      ...
      if (ok) Alert.alert("Done", "Rotean completed the action.");
      else Alert.alert("Cancelled", "Nothing was changed.");
    } catch (e: any) {
  ...
  return (
    <Screen>
      <ScrollView keyboardShouldPersistTaps="handled">
        ...
        <Text style={s.sub}>Tell Rotean what you need in normal language. It will prepare actions before changing anything.</Text>
        <TextInput value={request} onChangeText={setRequest} placeholder="Handle my Saturday…" placeholderTextColor={colors.inkMuted} multiline style={s.input} />
          ...
          <View style={s.result}>
            ...
            {plan.actions.map((action: any) => (
              <View key={action.id} style={s.action}>
                <Text style={s.badge}>{actionLabels[action.kind as keyof typeof actionLabels] || "Action"}</Text>
...
const s = StyleSheet.create({
  ...
  input: { minHeight: 120, marginTop: 20, padding: 16, borderRadius: 18, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line, color: colors.ink, textAlignVertical: "top", fontSize: 16 },
  button: { marginTop: 12, height: 52, borderRadius: 14, backgroundColor: colors.green, alignItems: "center", justifyContent: "center" },
  bt: { ...typography.h3, color: colors.white },
  result: { marginTop: 20 },
  summary: { ...typography.h2, color: colors.ink },
  action: { marginTop: 12, padding: 18, borderRadius: 18, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.line },
  ...
  confirm: { marginTop: 14, alignSelf: "flex-start", paddingHorizontal: 14, paddingVertical: 10, borderRadius: 11, backgroundColor: colors.lime },
