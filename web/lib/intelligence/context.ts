...
export async function getRoteanContext(): Promise<RoteanContext> {
  ...
  return {
    ...
    model,
    now: { iso: now.toISOString(), timezone, day_of_week: new Intl.DateTimeFormat("en", { weekday: "long", timeZone: timezone }).format(now) },
    upcoming_events: snapshot.calendar_events ?? [],
    open_tasks: (snapshot.tasks ?? []).filter((task) => task.status !== "completed" && task.status !== "done"),
    active_goals: (snapshot.goals ?? []).filter((goal) => goal.status !== "completed" && goal.status !== "done"),
  };
}
